...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        norm = torch.mean((x ** 2),dim=2,keepdim = True)
        output = (x/(torch.sqrt(norm + self.eps))) * self.weight
...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        self.inv_freq = self.inv_freq.to(xk.device)

        batch,num_heads,seq_len,head_dim = xq.size()

        # R shape -> (nh,T,dim,dim)
        R = torch.zeros((num_heads,seq_len, head_dim, head_dim),device = xk.device, requires_grad=False)

        
        pos = torch.arange(1, seq_len+1,device = xk.device, requires_grad=False).unsqueeze(1)
        
        slice_i = torch.arange(0, head_dim // 2, device = xk.device, requires_grad=False)
        theta = 10000. ** (-2.0 * (slice_i.float()) / head_dim) 
        pos_emb = pos * theta

        cos_value = torch.cos(pos_emb)
        sin_value = torch.sin(pos_emb)


        # Putting the 2x2 rotation matrices along the diagonal and keeping other values at 0
        R[:,:, 2*slice_i, 2*slice_i] = cos_value
        R[:,:, 2*slice_i, 2*slice_i+1] = -sin_value
        R[:,:, 2*slice_i+1, 2*slice_i] = sin_value
        R[:,:, 2*slice_i+1, 2*slice_i+1] = cos_value
        ...
        # xq_rot shape -> (nh,T,B,dim)

        xq_rot = xq_rot.transpose(1,2)
        xq_rot = xq_rot.transpose(0,1)

        # xq_rot shape -> (B,nh,T,dim)

        # For shape check comments refer to the same for xq as the shapes are same

        xk = xk.transpose(0,1)
        xk = xk.transpose(1,2)

        xk_rot = xk@R
        xk_rot = xk_rot.transpose(1,2)
        xk_rot = xk_rot.transpose(0,1)
    ...
    def forward(self, x):
        ...
        # DOUBT should I get q,k,v first and then split into heads (here) or split x first into num of heads and then run c_attn 

        head_embed = C //self.n_head

        q = q.view(B,T,self.n_head,head_embed)      # B,T,nh,hs
        q = q.transpose(1,2)                        # B,nh,T,hs

        k = k.view(B,T,self.n_head,head_embed)      # B,T,nh,hs
        k = k.transpose(1,2)                        # B,nh,T,hs

        v = v.view(B,T,self.n_head,head_embed)      # B,T,nh,hs
        ...
        if self.use_flash_attn:
            ...
            y = F.scaled_dot_product_attention(q,k,v,attn_mask = self.mask[:,:,:T,:T].to(dtype=torch.bool),dropout_p  = self.config.attn_pdrop)
        else:
            # Compute attention scores
            att = q @ (k.transpose(2,3))          # (B,nh,T,hs) x (B,nh,hs,T) = (B,nh,T,T)
            att = att/(head_embed ** 0.5)      # B,nh,T,T
            # Apply causal mask
            att.masked_fill_(self.mask[:,:,:T,:T] == 0, -torch.inf)
            att = F.softmax(att,dim = 3)

            # DOUBT where exactly to add attn_dropout. CLEARED put it here only.
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # DOUBT Do I apply residual connections here. Also where to norm.

        norm_out_1 = self.layer_norm_1(x)

        att_out = self.self_attention(norm_out_1)

        resid_out_1 = att_out + x

        norm_out_2 = self.layer_norm_1(resid_out_1)

        mlpf_out = self.mlpf(norm_out_2)

        out = mlpf_out + norm_out_2
...
class GPT(nn.Module):
    ...
    def generate(self, idx: torch.LongTensor, max_new_tokens: int, temperature:float = 1.0, do_sample:bool = False, top_k:int = None, top_p: float = 0.6):
        ...
        self.eval()   # Setting model to eval here to disable dropouts
        for _ in range(max_new_tokens):
            ...
            if not do_sample:
                ...
                if top_k is not None:
                    """
                    sorted_p,sorted_indices = torch.sort(logits,dim=-1)
                    sorted_p = sorted_p[:,:top_k]
                    sorted_indices = sorted_indices[:,:top_k]
                    random_indices = torch.randint(low=0,high=top_k,size=(sorted_p.size()[0],),device=idx.device)
                    sorted_p = sorted_p[torch.arange(sorted_p.size()[0]),random_indices]
                    sorted_indices = sorted_indices[torch.arange(sorted_indices.size()[0]),random_indices]
                    """
                    top_k = min(top_k, softmax_out.size(-1)) # top_k <= V
                    top_k_probs, top_k_indices = torch.topk(softmax_out, top_k, dim=-1)
                    softmax_out = torch.zeros_like(softmax_out).scatter_(-1, top_k_indices, top_k_probs)
                    softmax_out /= softmax_out.sum(dim=-1, keepdim=True)

                    

                # optionally apply top-p sampling
                if top_p is not None:
                    sorted_p,sorted_indices = torch.sort(softmax_out,dim=-1,descending = True)
                    cum_probs = sorted_p.cumsum(dim=-1)
                    invalid_indices_sorted = cum_probs > top_p

                    # Right shifting to avoid edge case when top_p is smaller than first probability
                    # Setting first column to 0
                    invalid_indices_sorted[..., 1:] = invalid_indices_sorted[:, :-1].clone()
                    invalid_indices_sorted[..., 0] = 0

                    # Set invalid indices (as outsipe top_p) to 0 probability
                    invalid_indices = invalid_indices_sorted.scatter(-1, sorted_indices, invalid_indices_sorted)
                    softmax_out[invalid_indices] = 0

                    softmax_out /= softmax_out.sum(dim=-1, keepdim=True)
                    

                idx_next = torch.multinomial(softmax_out, num_samples=1) # Sample 1 index from each of the row prob distributions
